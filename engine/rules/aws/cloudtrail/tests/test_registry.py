...
def test_cloudtrail_registry_contains_cloudwatch_logs_rule():
    rule = CLOUDTRAIL_RULES.get_rule("CS-AWS-CT-008")

    assert rule is not None
    assert rule.rule_id == "CS-AWS-CT-008"
    assert rule.name == "cloudtrail_cloudwatch_logs"
    assert rule.data_source == "cloudtrail_trails"
    assert rule.collection_mode == "multiple"


def test_cloudtrail_registry_cloudwatch_logs_rule_has_expected_arguments():
    rule = CLOUDTRAIL_RULES.get_rule("CS-AWS-CT-008")
...
def test_cloudtrail_registry_contains_event_data_store_encryption_rule():
    rule = CLOUDTRAIL_RULES.get_rule("CS-AWS-CT-010")

    assert rule is not None
    assert rule.rule_id == "CS-AWS-CT-010"
    assert rule.name == "cloudtrail_event_data_store_encryption"
    assert rule.data_source == "cloudtrail_event_data_stores"
    assert rule.collection_mode == "multiple"


def test_cloudtrail_registry_event_data_store_encryption_rule_has_expected_arguments():
    rule = CLOUDTRAIL_RULES.get_rule("CS-AWS-CT-010")
...
def test_cloudtrail_registry_contains_event_data_store_termination_protection_rule():
    rule = CLOUDTRAIL_RULES.get_rule("CS-AWS-CT-011")

    assert rule is not None
    assert rule.rule_id == "CS-AWS-CT-011"
    assert (
        rule.name
        == "cloudtrail_event_data_store_termination_protection"
    )
    assert rule.data_source == "cloudtrail_event_data_stores"
    assert rule.collection_mode == "multiple"


def test_cloudtrail_registry_event_data_store_termination_protection_rule_has_expected_arguments():
    rule = CLOUDTRAIL_RULES.get_rule("CS-AWS-CT-011")
